...
$re=$_GET['re'];
$a=$_COOKIE['core'];
$fql="SELECT name,uid,current_location,pic,username,sex,profile_url,mutual_friend_count,pic_square,online_presence FROM   user WHERE  CONTAINS  ('".$re."')AND  strpos(lower(name),lower('".$re."')) >=0  AND current_location AND uid IN (SELECT uid2 FROM friend WHERE uid1 =me()) AND current_location !='NULL' ORDER BY name";
...
<div id="l">Try again</div>
<br><span style="color:blue;">&copy; WebCored</span></div></center>';
}
?>
